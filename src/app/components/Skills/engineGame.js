...
if (ExecutionEnvironment.canUseDOM) {
  require('../../../libs/chessboard.min.js');
}

module.exports = function engineGame(options) {
  options = options || {}
  var game = new Chess();
  var board;
  ...
  function displayStatus() {
    var status = '<strong>Engine:</strong> ';
    ...
    status += ' <strong>Book:</strong> ' + engineStatus.book;
    if (engineStatus.search) {
      status += '<br>' + engineStatus.search;
      if (engineStatus.score && displayScore) {
        status += ', <strong>Score:</strong> ' + engineStatus.score;
      }
    }
    $('#engine-status').html(status);
  }
  ...
  engine.onmessage = function(event) {
    ...
    if (line == 'uciok') {
      ...
      if (match) {
        ...
        engineStatus.search = '<strong>Depth:</strong> ' + match[1] +
          ', <strong>Nps:</strong> ' + match[2];
      }
